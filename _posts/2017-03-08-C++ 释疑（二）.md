...
-------------------------- | -----------------------------------------
`setfill(ch)` | 用ch填充空白，通常与left,right,internal和setw(w)一起使用
`setprecision(n)` | 将浮点精度设置为n
`setw(w)` | 读或写值的宽度为w个字符，写时默认右对齐
`setbase(b)` | 将整数输出为b进制
...
float f = 1.243f;
auto dp = cout.precision();
cout << setprecision(12) << f << std::endl;
cout << setprecision(dp);
cout << setw(12) << 1234 << std::endl;
cout << setfill('*') << internal << setw(12) << -1234 << std::endl;
cout << setfill(' ');
```
...
`tellg()` | 返回输入流中标记的当前位置
`tellp()` | 返回输出流中标记的当前位置
`seekg(pos)` <br> `seekp(pos)` | 在输入流或输出流中将标记重定位到给定的绝对位置<br>pos通常是tellg或tellp返回的值
`seekp(off,from)` <br> `seekg(off,from)` | 在一个输入流或输出流中将标记定位到from之前或之后off个字符

```c++
fstream inOut("example.txt");
auto beg_mark = inOut.tellg();
inOut.seekg(0,fstream::end);
auto end_mark = inOut.tellg();
inOut.seekg(beg_mark);
//inOut.seekg(0,fstream::beg);
auto length = end_mark - beg_mark;
...
标准库定义了三个顺序容器适配器，这些适配器不能用于`array`和`forward_list`

*	stack
*	queue: 不能用于vector
*	priority_queue: 不能用于list

<h4 id="container_adapter_basic_operations">基本操作</h4>

所有容器适配器都支持的操作和类型 | 说明
------------------------------ | -------------------------------------------------
`size_type` | 一种无符号类型，足以保存当前类型的最大对象的大小
`value_type` | 元素类型
`container_type` | 实现适配器的底层容器类型
`A a` | 创建一个名为a的空适配器
`A a(c)` | 创建一个名为a的适配器，带有容器c的一个拷贝
`==,!=,<,<=,>,>=` | 关系运算符
`a.empty()` | 若a不包含任何元素，返回true
`a.size()` | 返回a中的元素数目
`swap(a,b)` <br> `a.swap(b)` | 交换a与b的内容，a与b必须有相同的类型，包括底层容器类型

<h4 id="container_adapter_stack">stack</h4>

stack定义于头文件`<stack>`，默认基于deque实现，采用先进后出策略

stack 操作 | 说明
-------------| -------------------------------------------------
`s.pop()` | 删除栈顶元素，返回void
`s.push(item)` <br> `s.emplace(args)` | 创建一个新元素压入栈顶
`s.top()` | 返回栈顶元素

<h4 id="container_adapter_queue">queue, priority_queue</h4>

`queue`, `priority_queue`定义于头文件`<queue>`，采用先进先出策略

*	queue默认基于`deque`实现；
*	`priority_queue`默认基于`vector`实现，使用`std::less`确定相对优先级，即**最大的排在最前面**。

操作 | 说明
------------------ | -------------------------------------------------
`q.pop()` | 删除首元素或最高优先级的元素，返回void
`q.front()` <br> `q.back()` | 返回首元素或尾元素，只适用于queue
`q.top()` | 返回最高优先级元素，只适用于`priority_queue`
`q.push(item)` <br> `q.emplace(args)` | 在queue末尾或`priority_queue`中恰当的位置创建一个元素
...
priority_queue<ListNode *, vector<ListNode *>, function<bool(ListNode *, ListNode *)>>
	pQueue([](ListNode *a, ListNode *b) {return a->val > b->val; });
```
...
`includes(b1,e1,b2,e2)` <br> `includes(b1,e1,b2,e2,func)` | 若[b2,e2)中的每个元素都包含在[b1,e1)中，则返回true
`lower_bound(b,e,val)` <br> `lower_bound(b,e,val,func)` | 返回一个迭代器，表示第一个小于等于val的元素<br>[b,e)必须是从小到大排序的<br>失败返回指向第一个大于val的迭代器或e
`upper_bound(b,e,val)` <br> `upper_bound(b,e,val,func)` | 返回一个迭代器，表示第一个大于val的元素<br>[b,e)必须是从小到大排序的<br>失败返回e
